...
import { ArrowLeft, Eye, EyeOff, LoaderCircle, LockKeyhole, Shell, Sparkles } from 'lucide-react'
...
export default function AccessPage() {
  ...
  const changeMode = (nextMode: AccessMode) => {
    ...
    setError('')
    setNotice('')
    const url = nextMode === 'register' ? '/acesso?modo=cadastro' : '/acesso'
    window.history.replaceState(null, '', url)
  }

  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#073f53] px-5 py-12">
      <img src="/hero-ocean.png" alt="" className="absolute inset-0 size-full object-cover opacity-35" />
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <section className="rounded-[2rem] border border-white/20 bg-[#fffdf7] p-7 shadow-2xl sm:p-9">
          <div className="mb-7 flex size-12 items-center justify-center rounded-2xl bg-[#dff1ed] text-[#0a7088]">
            {mode === 'register' ? <Sparkles size={23} /> : <LockKeyhole size={22} />}
          </div>
          <p className="eyebrow text-[#178ba4]">{mode === 'register' ? 'Primeiro acesso' : 'Área do responsável'}</p>
          <h1 className="mt-2 font-display text-4xl leading-tight text-[#073f53]">
            {isAdminRegistration ? 'Crie seu acesso administrativo' : mode === 'register' ? 'Crie seu convite' : 'Entre no seu painel'}
          </h1>
          ...
          <div className="mt-6 grid grid-cols-2 rounded-2xl bg-[#e8f3ef] p-1.5">
